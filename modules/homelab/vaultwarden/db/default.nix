{
  ...
  config = lib.mkIf config.homelab.vaultwarden.enable {
    services.k3s = {
      manifests.cnpg-databases.content =
        [
...

          {
            ...
            spec =
              {
                instances = config.homelab.vaultwarden.db.instances;
                minSyncReplicas = 0;
                maxSyncReplicas = 0;
                imageCatalogRef = {
                ...
                storage = {
                  ...
                  size = config.homelab.vaultwarden.db.storageSize;
                };

                walStorage = {
                  storageClass = "local-path";
                  resizeInUseVolumes = false;
                  size = config.homelab.vaultwarden.db.walStorageSize;
                };

                resources = config.homelab.vaultwarden.db.resources;
              }
              # TODO: Backups
              // lib.optionalAttrs config.homelab.vaultwarden.db.backup.enable {
                backup = {
                  barmanObjectStore = {
        ...
        ++ lib.optionals config.homelab.vaultwarden.db.backup.enable [
          {
            apiVersion = "postgresql.cnpg.io/v1";
      ...
      secrets = lib.mkIf config.homelab.vaultwarden.db.backup.enable [
        {
          metadata = {
            name = "vaultwarden-backup-credentials";
            namespace = "vaultwarden";
          ...
          stringData = {
            "access-key-id" = config.homelab.vaultwarden.db.backup.accessKeyId;
            "secret-access-key" = config.homelab.vaultwarden.db.backup.secretAccessKey;
          };
        }
      ];
    };
