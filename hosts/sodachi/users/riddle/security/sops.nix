{
  ...
  sops = {
    age = {
      sshKeyPaths = ["/var/lib/sops-nix/.ssh/riddle"];
      # Instructions:
      # mkdir -p /var/lib/sops-nix/age
      # age-keygen -o /var/lib/sops-nix/age/keys.txt
      # Fill in your secrets in YAML format
      # sudo sops --encrypt  --in-place --age $(sudo age-keygen -y /var/lib/sops-nix/age/key.txt) ~/.nixos-dotfiles/hosts/sodachi/users/riddle/secrets.yaml
    ...
    templates.git-user = {
      path = "/home/riddle/.config/git/user.gitconfig";
      mode = "0644";
      owner = "riddle";
      content = ''
        [user]
    ...
    templates.allowed-signers = {
      path = "/home/riddle/.config/git/allowed-signers";
      mode = "0644";
      owner = "riddle";
      content = ''
